...
class PokerGame:
    ...
    def betting_round(self, round_name, community, player_hand, pot):
        ...
        while True:
            user_input = input(f"Enter bet (Min: {min_bet}, Fold: 0): ")
            ...
            try:
                ...
                if bet_amount < min_bet:
                    print(f"Invalid! Bet must be at least {min_bet}.")
                    continue
                
                if bet_amount > self.player_chips:
                    print(f"You only have {self.player_chips} chips!")
                    continue
...
game = PokerGame()
print("Welcome to Terminal Texas Hold'em! - v3.0")
